...
function bars(n: number, seconds: number): number[] {
  let x = n * 9301 + 49297;
  const next = () => (x = (x * 9301 + 49297) % 233280) / 233280;
  const count = Math.round(Math.min(22, 7 + seconds * 1.5));
...
export function Wave({ n, seconds, arrived }: { n: number; seconds: number; arrived?: boolean }) {
  return (
...
interface Props {
  s: SceneState;
  seconds: number; // of narration: how wide its waveform is
  style: CSSProperties;
...
export default function ReelSlide({ s, seconds, style, lit, run, open, dim, ref, onOpen, onHover }: Props) {
