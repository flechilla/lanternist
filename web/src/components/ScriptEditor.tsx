...
export default function ScriptEditor(p: Props) {
  ...
  function renameCast(i: number) {
    ...
    edit((sb) => {
      ...
      sb.scenes.forEach((s) => {
        s.cast = s.cast.map((x) => (x === c.id ? id : x));
      });
      c.id = id;
    });
  }
    ...
    <>
      {p.dirty && (
        <div className="savebar" role="status">
          ...
          <button className="small" onClick={p.onDiscard} disabled={p.busy}>
            Discard
          </button>
          <button className="small primary" onClick={p.onSave} disabled={p.busy}>
            {p.busy ? "Saving…" : "Save"}
          </button>
        </div>
      )}
        ...
        <aside>
          <section className="panel stack">
            ...
            <Slide
              square
              image={p.cast}
              drawing={p.castDrawing}
              label="Cast sheet"
              empty="The cast sheet keeps every character looking the same in every picture."
            />
            <div className="row">
              {p.cast ? (
                <button className="small" onClick={p.onRerollCast} disabled={p.busy || p.jobActive}>
                  Draw it again
                </button>
              ) : (
                <button className="small primary" onClick={p.onDrawCast} disabled={p.busy || p.jobActive}>
                  Draw cast sheet
                </button>
              )}
            </div>
            {draft.cast_sheet_prompt !== null && draft.cast_sheet_prompt !== undefined && (
              <label className="field">
                ...
                <textarea
                  rows={6}
                  value={draft.cast_sheet_prompt}
                  onChange={(e) =>
                    edit((sb) => {
                      sb.cast_sheet_prompt = e.target.value;
                    })
                  }
                />
              </label>
          ...
          <section className="panel stack">
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(e) =>
                  edit((sb) => {
                    sb.subtitles = e.target.value as Storyboard["subtitles"];
                  })
                }
              >
              ...
              <div className="chips">
                {p.opts.styles.map((s) => (
                  <button
                    ...
                    type="button"
                    className="chip"
                    aria-pressed={draft.style === s.prompt}
                    onClick={() =>
                      edit((sb) => {
                        sb.style = s.prompt;
                      })
      ...
      <div className="stack">
        <label className="field narr">
          ...
          <textarea
            ...
            onChange={(e) =>
              scene(s.n, (x) => {
                x.narration = [{ speaker: "narrator", text: e.target.value }];
              })
            }
          />
        </label>
        <div className="scene-fields">
          <label className="field">
            ...
            <textarea
              rows={4}
              value={s.visual}
              placeholder="Shot, setting, light, who does what"
              onChange={(e) =>
                scene(s.n, (x) => {
                  x.visual = e.target.value;
                })
              }
            />
          </label>
          ...
          <label className="field">
            ...
            <textarea
              rows={4}
              value={s.sound}
              placeholder="Ambience only: wind, waves, birdsong"
              onChange={(e) =>
                scene(s.n, (x) => {
                  x.sound = e.target.value;
                })
              }
            />
          </label>
        ...
        <div className="scene-meta">
          ...
          <label className="row" style={{ gap: 6 }}>
            ...
            <select
              ...
              onChange={(e) =>
                scene(s.n, (x) => {
                  x.camera = e.target.value as Camera;
                })
              }
            >
              {(opts?.cameras ?? (Object.keys(CAMERA_NAMES) as Camera[])).map((c) => (
          ...
          <div className="stack" style={{ marginTop: 10 }}>
            <form
              ...
              onSubmit={(e) => {
                e.preventDefault();
                if (instruction.trim()) {
                  onRewrite(s.n, instruction.trim());
                  setInstruction("");
                }
              }}
            >
              <input
                ...
                placeholder="Make the gull a bit funnier"
                aria-label={`How to rewrite scene ${s.n}`}
              />
              <button type="submit" className="small" disabled={!instruction.trim() || jobActive}>
                Rewrite scene {s.n}
              </button>
            </form>
            <div className="row">
              <button type="button" className="small" onClick={() => onInsert(s.n)}>
                Add a scene after this one
              </button>
              {canRemove && (
                <button type="button" className="small danger" onClick={() => onRemove(s.n)}>
                  Remove scene {s.n}
                </button>
              )}
            </div>
