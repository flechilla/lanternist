...
export function jobTitle(job: Job): string {
  switch (job.kind) {
    case "write": return "Writing the story";
    case "rewrite": return `Rewriting scene ${job.params.n ?? ""}`;
    case "cast": return "Drawing the cast sheet";
    case "board": return "Preparing the board";
    case "render": return "Rendering the film";
  }
}
...
export function Stages({ job }: { job: Job }) {
  ...
  if (!shown.length) return <p className="muted">{job.status === "queued" ? "Waiting for the job ahead of it." : "Starting…"}</p>;
  return (
      ...
      {shown.map(([key, name]) => {
        ...
        return (
          <div key={key} className={`stage ${st.status}`}>
            ...
            {unknown ? (
              <span className="bar indeterminate" role="progressbar" aria-label={name}><i /></span>
            ) : (
              <span className="bar" role="progressbar" aria-label={name} aria-valuenow={pct} aria-valuemin={0} aria-valuemax={100}>
                <i style={{ width: `${pct}%` }} />
              </span>
            )}
            <span className="count">{st.total ? `${st.done}/${st.total}` : st.status === "done" ? "done" : "working"}</span>
          </div>
        );
      })}
    </div>
...
export function Log({ job }: { job: Job }) {
  ...
  return <pre ref={ref} className="log">{lines.join("\n")}</pre>;
}
  ...
  return (
    <section className="dock" aria-live="polite" aria-label="Job in progress">
      <div className="dock-head">
        ...
        <div className="what">
          <b>{jobTitle(job)}{queued > 0 ? ` (${queued} more queued)` : ""}</b>
          <span>{message}</span>
        </div>
        <button className="small" onClick={() => setOpen(!open)} aria-expanded={open}>
          {open ? "Hide details" : "Details"}
        </button>
        <button className="small danger" onClick={onCancel}>Cancel</button>
      </div>
      {open && (
        <div className="body">
          <Stages job={job} />
