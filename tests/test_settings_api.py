...
def test_keys_are_stored_but_never_returned(client):
    ...
    assert "fal-key-abcdef-1234" not in r.text and "fal-key-abcdef-1234" not in client.get("/api/providers").text
...
def test_settings_round_trip(client):
    ...
    assert rows["defaults.video"] == {"key": "defaults.video", "label": "Video model",
                                      "value": "local/ltx-2.5-22b-nvfp4", "source": "default"}
    r = client.put("/api/settings", json={"changes": {"defaults.video": "fal/kling-v3-standard",
                                                      "defaults.budget_usd": 2.5}})
    assert r.status_code == 200
    rows = {x["key"]: x for x in r.json()}
    assert rows["defaults.video"]["value"] == "fal/kling-v3-standard" and rows["defaults.video"]["source"] == "app"
    ...
    reset = {x["key"]: x for x in client.put("/api/settings", json={"changes": {"defaults.video": None}}).json()}
