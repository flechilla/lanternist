...
def spec(**kw) -> RunSpec:
    return RunSpec(
        **{
            "stage": "motion",
            "model_id": "fal/kling-v3-standard",
            "step_key": "key1",
            "unit": "output_second",
...
async def test_a_request_is_logged_priced_and_downloaded(fal, db, world, tmp_path):
    ...
    res = await fal.run(
        KLING,
        {"prompt": "a cat", "start_image_url": "https://x/y.png", "duration": "5"},
        spec(),
        on_status=lambda st, _: seen.append(st),
    )
    assert {"IN_QUEUE", "IN_PROGRESS"} <= set(seen) and not res.resumed
    assert res.billable_units == 5.0 and res.cost_micros == 420_000
    [run] = runs(db)
    assert (run.status, run.provider, run.request_id, run.cost_source) == (
        "done",
        "fal",
        res.request_id,
        "computed",
...
async def test_every_request_asks_fal_to_expire_its_media(cfg, db, world):
    ...
    assert json.loads(submit.headers["x-fal-object-lifecycle-preference"]) == {
...
async def test_rate_limits_and_server_errors_are_retried(fal, db, world, monkeypatch):
    ...
    world.fal.fail_submit += [
        (429, {"detail": "concurrent_requests_limit"}),
        (503, {"detail": "runner down"}),
    ]
    res = await fal.run(KLING, {"duration": "5"}, spec())
...
async def test_price_sync_records_changes_only(cfg, db, fal, world):
    ...
    assert (
        by_model["fal/veo-3.1-fast"]["drift"] is None
        and by_model["fal/veo-3.1-fast"]["status"] == "deprecated"
    )
...
async def test_plain_chat_doesnt_force_parameters(router, world):
    ...
    assert res.text.startswith("TITLE:") and world.openrouter.chats[0]["provider"] == {
...
async def test_rate_limits_and_errors_after_200_are_retried(router, world, monkeypatch):
    ...
    world.openrouter.replies += [
        (429, {"error": {"code": 429, "message": "slow down"}}),
        (200, {"error": {"code": 502, "message": "provider hiccup"}}),
        "fine",
    ]
...
def test_sample_fits_the_schema():
    s = {
        "type": "object",
        "properties": {
            "a": {"type": "array", "items": {"enum": ["x", "y"]}, "minItems": 2},
            "b": {"type": ["number", "null"]},
            "c": {"type": "boolean"},
        },
    }
